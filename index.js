...
function setToArray(mySet) {
  ...
  const arrFilter = arr.filter((value) => typeof value === "string" );
...
function getIntersection(arr1, arr2) {
  ...
  const intersectionSetSet = new Set();

  for (const value of set1) {
    if (set2.has(value)) {
      intersectionSetSet.add(value);
    }
  }
  return intersectionSetSet;
}
...
function iterateSet(set) {
  ...
  for (const valEnt of entriesSet) {
    console.log(valEnt);
  };
